...
// class EasyRtcAdapter extends NoOpAdapter {
  ...
  constructor(easyrtc) {
    ...
    this.easyrtc.setPeerOpenListener((clientId) => {
      ...
      for(let streamName in this.mediaStreams["local"]){
        if (this.easyrtc.getConnectStatus(clientId) !== this.easyrtc.NOT_CONNECTED) {
          this.easyrtc.addStreamToCall(clientId, streamName);
          console.log('adding screen share to clientId ',clientId,streamName)
        }else{
          console.log('not adding screen share to clientId ',clientId, 'because it not connected',streamName)
        }
      }
    });
  ...
  _connect(connectSuccess, connectFailure) {
    ...
    if (that.easyrtc.audioEnabled || that.easyrtc.videoEnabled) {
      navigator.mediaDevices.getUserMedia({
        ...
        audio: false //that.easyrtc.audioEnabled
      }).then(
        function(stream) {
